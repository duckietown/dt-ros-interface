...
This directory contains the camera transport benchmark tooling used to compare HTTP and SHM behavior in the ROS1 camera bridge.
...
- `capture-output`: measures the existing ROS1 output topic.
  This mode does not open the SHM FIFO and is the preferred way to compare HTTP and SHM throughput end to end.
  It reports output fps, camera-source-to-external-ROS1-subscriber delay, and container CPU and memory.
- `compare`: compares one HTTP and one SHM ROS1 output-capture result only.

The ROS1 and ROS2 benchmark tools are intentionally separate. Do not combine
their result files: this script accepts only schema-v2 ROS1 output captures
...
```bash
cd /home/ubuntu/duckietown/dt-ros-interface

python3 tests/benchmark_camera_transport.py capture-output \
  --transport http \
  --robot-name morpheus \
  --mode-container ros-interface \
  --containers driver-camera ros-interface \
  --output-json /tmp/ros1-camera-http-output.json
```
...
python3 tests/benchmark_camera_transport.py capture-output \
  ...
  --mode-container ros-interface \
  --containers driver-camera ros-interface \
  --output-json /tmp/ros1-camera-shm-output.json
```
...
python3 tests/benchmark_camera_transport.py compare \
  /tmp/ros1-camera-http-output.json \
  /tmp/ros1-camera-shm-output.json \
  --output-json /tmp/ros1-camera-output-compare.json
```

`capture-output` measures from the timestamp embedded by the camera producer
to receipt by the external ROS1 subscriber. It therefore includes DTPS,
bridge conversion, and ROS1 delivery. All participants must share a wall
...
- `capture-output` requires ROS1 Python packages such as `rospy` and `sensor_msgs`.
- Both capture modes require access to container stats through either the Docker CLI or the Python Docker SDK.

If your local shell does not have the required DTPS or ROS1 runtime available, run the script inside a helper container based on `duckietown/dt-ros-interface:ente-arm64v8` and mount the Docker socket. For direct HTTP capture, also mount the DTPS socket path.
...
For helper-container ROS1 output capture, you may also need to provide the standard ROS1 networking settings if they are not already present in the environment:
...
- `dtps_transport`, `ros_transport`, and `transport_path`: the DTPS mode and
  the fixed ROS1 output path for the run.
- `timestamp_origin` and `timestamp_clock`: the provenance and clock expected
  by the timing summary.
- `measurement_surface`: `transport_ingress` or `ros1_output_topic`
...
- The default ROS1 output topic observed by `capture-output` is `/<robot-name>/camera_node/image/compressed`.
