...
def _bootstrap_workspace_paths() -> None:
    ...
    candidates = [
        Path("/opt/ros") / ros_distro / "lib" / python_version / "dist-packages",
...
_ROS1_CAPTURE_IMPORTS: Optional[dict[str, Any]] = None
...
def _load_ros1_capture_imports() -> dict[str, Any]:
    global _ROS1_CAPTURE_IMPORTS
    if _ROS1_CAPTURE_IMPORTS is not None:
        return _ROS1_CAPTURE_IMPORTS
    try:
        import rospy
        from sensor_msgs.msg import CompressedImage as ROSCompressedImage
    except ModuleNotFoundError as exc:
        raise RuntimeError(
            "ROS1 output capture requires rospy and sensor_msgs in the runtime environment."
        ) from exc

    _ROS1_CAPTURE_IMPORTS = {
        "ROSCompressedImage": ROSCompressedImage,
        "rospy": rospy,
    }
    return _ROS1_CAPTURE_IMPORTS
...
def _ensure_ros_runtime_environment() -> None:
    if os.environ.get("DT_CAMERA_BENCHMARK_ROS_ENV_READY") == "1":
        return
    if os.environ.get("ROS_PACKAGE_PATH"):
        return
...
class TopicFrameSink:
    ...
    def on_ros1_frame(self, payload_size_bytes: int, header_stamp_sec: Optional[float]) -> None:
        loop = self._loop
        if loop is None:
            return
        received_at = time.time()
...
async def _sample_resources(
    ...
    while not stop_event.is_set():
        ...
        samples = await _run_in_thread(_read_docker_stats, containers)
...
def _stamp_to_seconds(stamp: Any) -> Optional[float]:
    ...
    if callable(to_sec):
        return _timestamp_to_seconds(to_sec())
    ...
    sec_value = float(sec)
    if nsecs is None:
        nsecs = getattr(stamp, "nanosec", 0)
    nsecs_value = float(0 if nsecs is None else nsecs)
    return _timestamp_to_seconds(
        sec_value + (nsecs_value / 1_000_000_000.0),
    )
...
async def _stream_ros1_topic_frames(
    topic_name: str,
    ros_master_uri: str,
    ros_ip: str,
    ros_hostname: str,
    ...
    imports = _load_ros1_capture_imports()
    rospy = imports["rospy"]
    compressed_image_cls = imports["ROSCompressedImage"]
    ...
    is_initialized = getattr(getattr(rospy, "core", None), "is_initialized", None)
    ...
    if not initialized:
        rospy.init_node("camera_transport_benchmark", anonymous=True, disable_signals=True)

    def _callback(message: Any) -> None:
        header = getattr(message, "header", None)
        stamp = getattr(header, "stamp", None)
        header_stamp_sec = _stamp_to_seconds(stamp)
        payload_size_bytes = len(message.data)
        sink.on_ros1_frame(payload_size_bytes, header_stamp_sec)
    ...
    try:
        ...
        if not rospy.is_shutdown():
            rospy.signal_shutdown("camera transport benchmark complete")
...
async def _collect_frame_metrics(
    ...
    while True:
        ...
        timestamp_status = _append_timestamp_latency(
            _timestamp_to_seconds(jpeg.header.timestamp),
            frame.received_at,
...
async def _collect_output_topic_metrics(
    ...
    if measured_frame_count == 0:
        raise RuntimeError(
            "No ROS1 topic frames were captured during the measurement window. "
            "Increase --startup-timeout/--warmup or check the ROS1 topic name and master settings."
        )
...
def _print_capture_summary(result: dict[str, Any]) -> None:
    ...
    if measurement_surface == "ros1_output_topic":
        source_to_output = frame_metrics.get(
            "source_to_ros_output_receive_ms",
            {},
        )
        _log(
            "Camera source->ROS1 receive ms: "
...
def _build_capture_result(
    ...
    if measurement_surface == "ros1_output_topic":
        ...
        transport_path = f"DTPS {args.transport.upper()} -> ROS 1"
    else:
        ros_transport = "none"
...
async def _run_output_capture(args: argparse.Namespace) -> dict[str, Any]:
    ...
    resource_metrics = _summarize_resources(resource_samples, window)
    notes = [
        "ROS1 output capture observes the published ROS topic instead of opening the SHM FIFO directly.",
    ]
    return _build_capture_result(
        args,
        capture_started_at,
        ...
        resource_metrics,
        window,
        measurement_surface="ros1_output_topic",
        observed_topic=observed_topic,
        notes=notes,
    )
...
def _get_timing_summary(payload: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    """Return the timing metric whose boundary matches a ROS1 capture result."""
    ...
    elif measurement_surface == "ros1_output_topic":
...
def _validate_comparison_result(result: dict[str, Any]) -> None:
    """Ensure a result belongs to this ROS1 benchmark and timing contract."""
    ...
    if result.get("measurement_surface") != "ros1_output_topic":
        raise ValueError(
            "Compare requires ROS1 output-topic captures; use capture-output "
            "for both the HTTP and SHM result."
        )
    if result.get("ros_transport") != _ROS_TRANSPORT:
        raise ValueError(
            "Result has an unexpected ROS transport for this ROS1 benchmark: "
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "then compare HTTP and SHM result files for the ROS1 camera bridge."
        )
    )
    ...
    capture_parser.add_argument(
        "--containers",
        nargs="*",
        default=["driver-camera", "ros-interface"],
        help="Container names to sample with docker stats",
    )
    capture_parser.add_argument(
        "--mode-container",
        default="ros-interface",
    ...
    output_parser = subparsers.add_parser(
        "capture-output",
        help="Capture throughput from the ROS1 output topic without opening the SHM transport directly",
    )
    ...
    output_parser.add_argument("--ros-master-uri", default="")
    output_parser.add_argument("--ros-ip", default="")
    output_parser.add_argument("--ros-hostname", default="")
    ...
    compare_parser = subparsers.add_parser(
        "compare",
        help="Compare one HTTP and one SHM result file from the ROS1 benchmark",
    )
